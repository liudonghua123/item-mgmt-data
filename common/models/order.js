...
module.exports = function(Order) {
  ...
  Order.observe('before save', function updateItemIfNecessary(ctx, next) {
    ...
    Order.findOne({where: {id: instance.id}}, function(err, order) {
      if (err) {
        console.error(err);
        next(err);
      }
      const previousIsSended = order.isSended;
      debug('previous isSended is %s', previousIsSended);
      if (!previousIsSended && instance.isSended) {
